...
def process_blocks(blocks: List[str], start_indent: int, comment: str) -> List[str]:
    ...
    for block in blocks:
        ...
        for line in block.splitlines():
            ...
            if line.strip() == "A:" or count_leading_spaces(line) < start_indent or is_sentence(line, comment):
...
def main() -> str:
    blocks = process_blocks(process_output(request.form.get("input"), get_output(process_input(request.form.get("input"), request.form.get("language"))).split("<|endoftext|>")[0], request.form.get("language")), count_leading_spaces(request.form.get("input").splitlines()[-1]), comments[request.form.get("language")]) # Get the output for the processed input

    return str([block.encode() for block in blocks])
